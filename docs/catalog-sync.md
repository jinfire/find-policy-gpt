...
적재 후 `npm run dev`로 앱을 열면 결과 화면의 전체 조건 후보와 홈 하단의 원본 검색에서 정책을 찾을 수 있다. 10개 정밀 추천, 전체 1차 후보, 원문 검색은 화면에서 구분한다.
...
- 새로 수집된 정책은 공식 지원조건을 `eligibility_profile`로 정규화해 `partially_structured`로 저장한다.
- 같은 동기화 파일을 다시 적용해도 동기화 이력과 정책 레코드를 멱등 upsert한다.
- 사람 검수와 정책별 룰 테스트를 통과해야 `rule_ready`가 된다.
